...
import {
  ...
  Shield, User as UserIcon, MapPin, Loader2, RefreshCw,
} from 'lucide-react';
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
            ...
            {formErr && (
              <p className="text-xs text-red-600 bg-red-50 p-2 rounded-lg mb-4">{formErr}</p>
            )}
              ...
              {(['name', 'email', 'region', 'phone'] as const).map(field => (
                <div key={field}>
                  ...
                  <Input
                    ...
                    className="bg-slate-50 border-slate-200 text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400"
            ...
            <div className="flex gap-3 mt-6">
              ...
              <Button
                ...
                className="flex-1 bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-60 flex items-center justify-center gap-2"
              >
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex gap-3">
          <div className="relative w-72">
            ...
            <Input
              className="pl-9 bg-white border-slate-200 rounded-lg text-sm shadow-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400"
          ...
          <Button
            ...
            size="icon"
            onClick={() => load(page)}
            className="bg-white border-slate-200 text-slate-400 hover:text-blue-600 shadow-sm"
          >
            <RefreshCw className="w-4 h-4" />
          </Button>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" className="flex items-center gap-2 text-slate-600 shadow-sm text-sm font-medium">
            <Filter className="w-4 h-4" /> Filter
          </Button>
          <Button
            className="flex items-center gap-2 bg-blue-600 text-white hover:bg-blue-700 shadow-sm text-sm font-medium"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                {['User', 'Role', 'Jurisdiction', 'Status', 'Last Active', 'Actions'].map((col, i) => (
                  <th
                    key={col}
                    className={`py-3 px-6 text-[11px] font-bold text-slate-500 uppercase tracking-wider${i === 5 ? ' text-right' : ''}`}
                  >
                    {col}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                [1,2,3,4,5].map(i => (
                  <tr key={i}>
                    {[1,2,3,4,5,6].map(j => (
                      <td key={j} className="py-4 px-6"><Skeleton className="h-5 rounded-md" /></td>
                    ))}
                  </tr>
                ))
              ) : filtered.length === 0 ? (
        ...
        <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <div className="flex gap-1">
            <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)}
              className="text-xs font-medium text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed">
              Previous
            </Button>
