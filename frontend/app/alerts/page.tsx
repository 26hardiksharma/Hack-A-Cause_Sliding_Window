...
import { Send, Users, MapPin, FileText, Clock, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
import { api, type SMSCampaign } from '@/lib/api';

const DISTRICT_OPTIONS = [
  { id: 1, name: 'Beed' }, { id: 2, name: 'Latur' }, { id: 3, name: 'Osmanabad' },
  { id: 4, name: 'Solapur' }, { id: 5, name: 'Aurangabad' }, { id: 6, name: 'Nanded' },
  { id: 7, name: 'Jalgaon' }, { id: 8, name: 'Buldhana' }, { id: 9, name: 'Washim' },
  { id: 10, name: 'Yavatmal' },
];
...
export default function AlertsPage() {
  ...
  const [district, setDistrict] = useState('0');         // '0' = all
  ...
  async function handleSend() {
    ...
    try {
      ...
      setToast({ ok: true, msg: `✅ ${res.sent} messages sent (${res.failed} failed). Campaign: ${res.campaign_id.slice(0, 8)}…` });
    } catch (e: unknown) {
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {(['farmers', 'authorities', 'all'] as const).map(g => (
                  <label key={g} className="cursor-pointer">
                    <input type="radio" name="target" className="peer sr-only" checked={target === g} onChange={() => setTarget(g)} />
                    <div className="p-3 rounded-xl border border-slate-200 text-center hover:bg-slate-50 peer-checked:border-blue-600 peer-checked:bg-blue-50 peer-checked:text-blue-700 transition-all">
                      ...
                      <span className="text-sm font-medium capitalize">{g === 'all' ? 'All Users' : g.charAt(0).toUpperCase() + g.slice(1)}</span>
                    </div>
                  </label>
                ))}
              </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={`text-xs ${message.length > 160 ? 'text-red-500 font-bold' : 'text-slate-400'}`}>{message.length}/160</span>
              </div>
              <textarea
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-100">
                  <div>
                    ...
                    <p className="text-sm font-bold text-slate-800">
                      {c.recipients.toLocaleString()} <span className="text-[10px] font-normal text-slate-500">{c.target_group}</span>
                    </p>
                  </div>
                  <div>
