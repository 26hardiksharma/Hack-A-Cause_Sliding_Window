  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col z-20 shadow-sm hidden md:flex h-full">
      ...
      <nav className="flex-1 overflow-y-auto px-3 space-y-1">
        <div className="px-3 pt-2 pb-2 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
          Main Menu
        </div>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon
                className={`w-5 h-5 mr-3 ${isActive ? "text-blue-600" : "text-slate-400 group-hover:text-blue-500"}`}
              />
              <span>{item.name}</span>
            </Link>
          );
